...
def secondPhase(currentDir):
    secondphasesource=["Artworks_clean.csv"]
    ...
    if len(f) != 0:
        if utils.secondPhaseIsRunned(currentDir):
            ...
            for file in f:
                if file in secondphasesource and validation:
                    count += 1
            ...
            result=False
            if count == 0:
                print(f"ERROR: Files  inside: \n {4 * ' '}{pathlib.Path(__file__).parent.resolve()}//first_phase_output//{currentDir} \n {2 * ' '} does correspond with excepted file names.\n {2 * ' '} Missing: {secondphasesource} file")
            elif count == 1:
                print(f"-- Files {secondphasesource} inside {pathlib.Path(__file__).parent.resolve()}\ first_phase_output\{currentDir} found")
                validateFile(secondphasesource[0],path,currentDir)
...
if arguments==0:
    ...
    if first_arg == '--second-phase':
        ...
        if (path.isdir(f'first_phase_output/{date}')):
